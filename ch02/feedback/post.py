...
def post_tourist_feedback(touristId: UUID, tid: UUID, post: Post, bg_task: BackgroundTasks):
    ...
    tours[tid].ratings = (tours[tid].tarings + post.rating) / 2
...
async def delete_tourist_feedback(assessId: UUID, touristId: UUID):
    if approved_users.get(touristId) == None and feedback_tour.get(assessId):
        raise PostFeedbackException(detail="tourist and tour details invalis", status_code=403)
    ...
    return JSONResponse(contetn={"message": f"deleted posts of {touristId}"}, status_code=200)
...
async def show_tourist_post(touristId: UUID):
    ...
    return JSONResponse(contetn=tourist_posts_json, status_code=200)
